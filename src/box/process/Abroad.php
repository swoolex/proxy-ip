...
class Abroad extends AbstractProcess
{
    /**
     * 进程名称
     */
    public $name = 'abroad-process';
    ...
    */
    ...
    public function run() {
        ...
        $Db = new \x\Db();
        $list = $Db->name('abroad_ip_verify')->order('update_time ASC')->limit($num)->select();
        ...
        foreach ($list as $k=>$v) {
            ...
            go(function () use ($channel, $k, $v, $http, $https, $Db){
                ...
                if ($code == 200) {
                    $Db->name('abroad_ip_verify')->where('id', $v['id'])->delete();
                    $Db->name('abroad_ip')->insert([
                        'ip' => $v['ip'],
                        'port' => $v['port'],
